...
- **主动问别人 (caller)**: 任何时候都能用本 skill 去问/委派/查另一个 session
  的状态 —— 只要知道对方的 session id 和它所在机器的 host。
...
# ask something and wait for the reply:
echo "你现在在干啥?进度如何?" | python3 "$PY" --to <PEER_SID> --host <IP> --from <MY_SID>
```
...
(default: local) · `--token` (default: from `~/.claude/cc_web.conf`) ·
...
- cc-web must be running on the peer's machine; pass its Tailscale/LAN IP as
  `--host` (`--host` defaults to this machine's own IP, for a same-machine peer).
